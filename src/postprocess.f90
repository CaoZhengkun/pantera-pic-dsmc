...
MODULE postprocess
   ...
   USE fields
   USE surface_emission_utils, ONLY: SEEM_OK, compute_surface_current_density
   ...
   SUBROUTINE GRID_AVG
      ...
      REAL(KIND=8), DIMENSION(:), ALLOCATABLE :: TIMESTEP_E2X
      ...
      IF (PIC_TYPE .NE. NONE .AND. DIMS == 1) THEN
         IF (GRID_TYPE == UNSTRUCTURED) THEN
            TIMESTEP_E2X = E_FIELD(1,1,:)**2
         ELSE
            ! Structured 1D E is nodal (j=0); store one E^2 value per cell.
            TIMESTEP_E2X = 0.5d0*(E_FIELD(1,0,0:NX-1)**2 + E_FIELD(1,0,1:NX)**2)
         END IF
      END IF
      ...
      IF (PIC_TYPE .NE. NONE .AND. DIMS == 1) AVG_E2X = (AVG_E2X*DBLE_AVG_CUMULATED + TIMESTEP_E2X)/(AVG_CUMULATED + 1.)
      ...
      IF (PIC_TYPE .NE. NONE .AND. DIMS == 1) DEALLOCATE(TIMESTEP_E2X)
      ...
      IF (PROC_ID .EQ. 0) THEN
         ...
         IF (BOOL_BINARY_OUTPUT) THEN
            ...
            IF (PIC_TYPE .NE. NONE) THEN
               IF (GRID_TYPE == UNSTRUCTURED) THEN
                  ...
                  WRITE(54321) 'E2_X '//ITOA(1)//' '//ITOA( NCELLS )//' double'//ACHAR(10)
                  WRITE(54321) AVG_E2X, ACHAR(10)
         ...
         ELSE  ! Write ASCII output.
            ...
            ELSE IF (DIMS == 1) THEN
               ...
               WRITE(54321,'(A,I10,I10,I10)') 'DIMENSIONS', NX+1, 1, 1
   ...
   SUBROUTINE TALLY_PARTICLE_TO_BOUNDARY(REFLECTED, PART, IC, IFACE)
      ...
      INTEGER :: INDEX, FACE_PG, DIRECTION, SOURCE_INDEX
      ...
      IF (DIMS == 1) THEN
         FACE_PG = U1D_GRID%CELL_EDGES_PG(IFACE, IC)
      ELSE IF (DIMS == 2) THEN
         FACE_PG = U2D_GRID%CELL_EDGES_PG(IFACE, IC)
      ELSE
         FACE_PG = U3D_GRID%CELL_FACES_PG(IFACE, IC)
      END IF

      IF (EXTRACTION_GROUP > 0 .AND. FACE_PG == EXTRACTION_GROUP .AND. &
          ALLOCATED(EXTRACTION_COUNT) .AND. PART%S_ID >= 1 .AND. PART%S_ID <= N_SPECIES) THEN
         SOURCE_INDEX = PART%SOURCE_TAG + 1
         IF (SOURCE_INDEX < 1 .OR. SOURCE_INDEX > PARTICLE_SOURCE_TAG_COUNT) &
            SOURCE_INDEX = PARTICLE_SOURCE_UNKNOWN + 1
         IF (REFLECTED) THEN
            DIRECTION = 1
         ELSE
            DIRECTION = 2
         END IF
         EXTRACTION_COUNT(DIRECTION, SOURCE_INDEX, PART%S_ID) = &
            EXTRACTION_COUNT(DIRECTION, SOURCE_INDEX, PART%S_ID) + 1_8
         EXTRACTION_WEIGHT(DIRECTION, SOURCE_INDEX, PART%S_ID) = &
            EXTRACTION_WEIGHT(DIRECTION, SOURCE_INDEX, PART%S_ID) + FNUM*SPWT
         EXTRACTION_CHARGE(DIRECTION, SOURCE_INDEX, PART%S_ID) = &
            EXTRACTION_CHARGE(DIRECTION, SOURCE_INDEX, PART%S_ID) + QE*SPECIES(PART%S_ID)%CHARGE*FNUM*SPWT
      END IF
   ...
   SUBROUTINE SURFACE_EMISSION_SAVE

      IMPLICIT NONE
      INTEGER :: N_SOURCES, N_SEE, DATA_COUNT, I, OFFSET, UNIT, IOS, WRITE_OK, IERR_DIAG
      INTEGER :: WINDOW_STEPS
      REAL(KIND=8) :: WINDOW_SECONDS, SOURCE_WEIGHT, SOURCE_CHARGE, SEE_EVENTS
      REAL(KIND=8) :: INCIDENT_WEIGHT, INCIDENT_ENERGY_SUM, EMITTED_MACROS, EMITTED_WEIGHT
      REAL(KIND=8) :: OUT_OF_RANGE, MEAN_ENERGY, WEIGHTED_YIELD, CURRENT_DENSITY, SIGNED_CHARGE
      REAL(KIND=8), DIMENSION(:), ALLOCATABLE :: LOCAL_DATA, TOTAL_DATA
      CHARACTER(LEN=512) :: FILE_NAME
      LOGICAL :: FILE_EXISTS

      N_SOURCES = N_SURFACE_CURRENT_EMIT_TASKS
      N_SEE = N_SECONDARY_EMISSION_MODELS
      IF (N_SOURCES <= 0 .AND. N_SEE <= 0) RETURN
      IF (SURFACE_DIAGNOSTIC_WINDOW_STEPS <= 0) RETURN

      DATA_COUNT = 2*N_SOURCES + 6*N_SEE
      ALLOCATE(LOCAL_DATA(MAX(1,DATA_COUNT)), TOTAL_DATA(MAX(1,DATA_COUNT)))
      LOCAL_DATA = 0.d0
      TOTAL_DATA = 0.d0
      DO I = 1, N_SOURCES
         OFFSET = 2*(I-1)
         LOCAL_DATA(OFFSET+1) = REAL(SURFACE_CURRENT_EMIT_TASKS(I)%MACRO_COUNT_WINDOW, KIND=8)
         LOCAL_DATA(OFFSET+2) = SURFACE_CURRENT_EMIT_TASKS(I)%CHARGE_WINDOW
      END DO
      DO I = 1, N_SEE
         OFFSET = 2*N_SOURCES + 6*(I-1)
         LOCAL_DATA(OFFSET+1) = REAL(SECONDARY_EMISSION_MODELS(I)%INCIDENT_EVENT_COUNT_WINDOW, KIND=8)
         LOCAL_DATA(OFFSET+2) = SECONDARY_EMISSION_MODELS(I)%INCIDENT_WEIGHT_WINDOW
         LOCAL_DATA(OFFSET+3) = SECONDARY_EMISSION_MODELS(I)%INCIDENT_ENERGY_WEIGHTED_WINDOW
         LOCAL_DATA(OFFSET+4) = REAL(SECONDARY_EMISSION_MODELS(I)%EMITTED_MACRO_COUNT_WINDOW, KIND=8)
         LOCAL_DATA(OFFSET+5) = SECONDARY_EMISSION_MODELS(I)%EMITTED_WEIGHT_WINDOW
         LOCAL_DATA(OFFSET+6) = REAL(SECONDARY_EMISSION_MODELS(I)%OUT_OF_RANGE_COUNT, KIND=8)
      END DO

      IF (DATA_COUNT > 0) &
         CALL MPI_REDUCE(LOCAL_DATA, TOTAL_DATA, DATA_COUNT, MPI_DOUBLE_PRECISION, MPI_SUM, 0, &
                         MPI_COMM_WORLD, ierr)

      WRITE_OK = 1
      IF (PROC_ID == 0) THEN
         FILE_NAME = TRIM(ADJUSTL(BOUNDARY_SAVE_PATH)) // 'surface_emission.dat'
         INQUIRE(FILE=TRIM(FILE_NAME), EXIST=FILE_EXISTS)
         OPEN(NEWUNIT=UNIT, FILE=TRIM(FILE_NAME), STATUS='UNKNOWN', POSITION='APPEND', &
              ACTION='WRITE', IOSTAT=IOS)
         IF (IOS /= 0) THEN
            WRITE(*,*) '  [WARNING] Cannot open surface emission diagnostics: ', TRIM(FILE_NAME)
            WRITE_OK = 0
         ELSE
            IF (.NOT. FILE_EXISTS) THEN
               WRITE(UNIT,'(A)') '# Positive current is defined as charge current directed into the computational domain.'
               WRITE(UNIT,'(A)') '# tID type physical_group cell face incident_species product_species area_m2 window_s '// &
                    'incident_events incident_weight absorbed_weight reflected_weight emitted_macro_count '// &
                    'emitted_weight target_current_A_m2 target_current_A actual_current_A_m2 actual_current_A '// &
                    'actual_signed_charge_C fractional_residual_macro_count mean_incident_energy_eV '// &
                    'weighted_yield out_of_range_samples'
            END IF
            WINDOW_STEPS = MAX(1, SURFACE_DIAGNOSTIC_WINDOW_STEPS)
            WINDOW_SECONDS = REAL(WINDOW_STEPS, KIND=8)*DT
            DO I = 1, N_SOURCES
               OFFSET = 2*(I-1)
               SOURCE_WEIGHT = FNUM*SPECIES(SURFACE_CURRENT_EMIT_TASKS(I)%SPECIES_ID)%SPWT
               IF (BOOL_RADIAL_WEIGHTING) SOURCE_WEIGHT = &
                    CELL_FNUM(SURFACE_CURRENT_EMIT_TASKS(I)%IC)* &
                    SPECIES(SURFACE_CURRENT_EMIT_TASKS(I)%SPECIES_ID)%SPWT
               SOURCE_CHARGE = SPECIES(SURFACE_CURRENT_EMIT_TASKS(I)%SPECIES_ID)%CHARGE
               CALL COMPUTE_SURFACE_CURRENT_DENSITY(TOTAL_DATA(OFFSET+2), &
                    SURFACE_CURRENT_EMIT_TASKS(I)%FACE_AREA, WINDOW_SECONDS, CURRENT_DENSITY, IERR_DIAG)
               IF (IERR_DIAG /= SEEM_OK) THEN
                  WRITE(*,*) '  [WARNING] Invalid fixed-source surface-current diagnostic at face ', &
                       SURFACE_CURRENT_EMIT_TASKS(I)%IFACE
                  WRITE_OK = 0
                  EXIT
               END IF
               WRITE(UNIT,'(I0,1X,A,1X,A,1X,I0,1X,I0,1X,A,1X,A,1X,ES24.16,1X,ES24.16,1X,14(ES24.16,1X),I0)') &
                    tID, 'fixed_current', &
                    '"'//TRIM(GRID_BC(SURFACE_CURRENT_EMIT_TASKS(I)%PHYSICAL_GROUP)%PHYSICAL_GROUP_NAME)//'"', &
                    SURFACE_CURRENT_EMIT_TASKS(I)%IC, SURFACE_CURRENT_EMIT_TASKS(I)%IFACE, &
                    '"'//TRIM(SPECIES(SURFACE_CURRENT_EMIT_TASKS(I)%SPECIES_ID)%NAME)//'"', '-', &
                    SURFACE_CURRENT_EMIT_TASKS(I)%FACE_AREA, WINDOW_SECONDS, &
                    0.d0, 0.d0, 0.d0, 0.d0, TOTAL_DATA(OFFSET+1), TOTAL_DATA(OFFSET+1)*SOURCE_WEIGHT, &
                    SIGN(SURFACE_CURRENT_EMIT_TASKS(I)%CURRENT_DENSITY, SOURCE_CHARGE), &
                    SIGN(SURFACE_CURRENT_EMIT_TASKS(I)%CURRENT_DENSITY, SOURCE_CHARGE)* &
                         SURFACE_CURRENT_EMIT_TASKS(I)%FACE_AREA, &
                    CURRENT_DENSITY, TOTAL_DATA(OFFSET+2)/WINDOW_SECONDS, TOTAL_DATA(OFFSET+2), &
                    SURFACE_CURRENT_EMIT_TASKS(I)%FRACTIONAL_RESIDUAL, -1.d0, 0.d0, 0
            END DO
            DO I = 1, N_SEE
               OFFSET = 2*N_SOURCES + 6*(I-1)
               SEE_EVENTS = TOTAL_DATA(OFFSET+1)
               INCIDENT_WEIGHT = TOTAL_DATA(OFFSET+2)
               INCIDENT_ENERGY_SUM = TOTAL_DATA(OFFSET+3)
               EMITTED_MACROS = TOTAL_DATA(OFFSET+4)
               EMITTED_WEIGHT = TOTAL_DATA(OFFSET+5)
               OUT_OF_RANGE = TOTAL_DATA(OFFSET+6)
               MEAN_ENERGY = 0.d0
               IF (INCIDENT_WEIGHT > 0.d0) MEAN_ENERGY = INCIDENT_ENERGY_SUM/INCIDENT_WEIGHT
               WEIGHTED_YIELD = 0.d0
               IF (INCIDENT_WEIGHT > 0.d0) WEIGHTED_YIELD = EMITTED_WEIGHT/INCIDENT_WEIGHT
               SIGNED_CHARGE = QE*(- &
                    SPECIES(SECONDARY_EMISSION_MODELS(I)%INCIDENT_SPECIES_ID)%CHARGE*INCIDENT_WEIGHT + &
                    SPECIES(SECONDARY_EMISSION_MODELS(I)%ELECTRON_SPECIES_ID)%CHARGE*EMITTED_WEIGHT)
               CALL COMPUTE_SURFACE_CURRENT_DENSITY(SIGNED_CHARGE, &
                    SECONDARY_EMISSION_MODELS(I)%SURFACE_AREA, WINDOW_SECONDS, CURRENT_DENSITY, IERR_DIAG)
               IF (IERR_DIAG /= SEEM_OK) THEN
                  WRITE(*,*) '  [WARNING] Invalid SEE surface-current diagnostic for model ', I
                  WRITE_OK = 0
                  EXIT
               END IF
               WRITE(UNIT,'(I0,1X,A,1X,A,1X,I0,1X,I0,1X,A,1X,A,1X,ES24.16,1X,ES24.16,1X,14(ES24.16,1X),I0)') &
                    tID, 'SEE', &
                    '"'//TRIM(GRID_BC(SECONDARY_EMISSION_MODELS(I)%PHYSICAL_GROUP)%PHYSICAL_GROUP_NAME)//'"', &
                    -1, -1, '"'//TRIM(SPECIES(SECONDARY_EMISSION_MODELS(I)%INCIDENT_SPECIES_ID)%NAME)//'"', &
                    '"'//TRIM(SPECIES(SECONDARY_EMISSION_MODELS(I)%ELECTRON_SPECIES_ID)%NAME)//'"', &
                    SECONDARY_EMISSION_MODELS(I)%SURFACE_AREA, WINDOW_SECONDS, SEE_EVENTS, &
                    INCIDENT_WEIGHT, INCIDENT_WEIGHT, 0.d0, EMITTED_MACROS, EMITTED_WEIGHT, &
                    0.d0, 0.d0, CURRENT_DENSITY, CURRENT_DENSITY* &
                    SECONDARY_EMISSION_MODELS(I)%SURFACE_AREA, SIGNED_CHARGE, 0.d0, &
                    MEAN_ENERGY, WEIGHTED_YIELD, NINT(OUT_OF_RANGE, KIND=8)
            END DO
            CLOSE(UNIT)
         END IF
      END IF
      CALL MPI_BCAST(WRITE_OK, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
      IF (WRITE_OK /= 1) THEN
         CALL ERROR_ABORT('Surface emission diagnostics failed on the master rank; '// &
              'aborting to avoid losing or duplicating a statistics window.')
         RETURN
      END IF

      DO I = 1, N_SOURCES
         SURFACE_CURRENT_EMIT_TASKS(I)%MACRO_COUNT_WINDOW = 0_8
         SURFACE_CURRENT_EMIT_TASKS(I)%CHARGE_WINDOW = 0.d0
      END DO
      DO I = 1, N_SEE
         SECONDARY_EMISSION_MODELS(I)%INCIDENT_EVENT_COUNT_WINDOW = 0_8
         SECONDARY_EMISSION_MODELS(I)%INCIDENT_WEIGHT_WINDOW = 0.d0
         SECONDARY_EMISSION_MODELS(I)%INCIDENT_ENERGY_WEIGHTED_WINDOW = 0.d0
         SECONDARY_EMISSION_MODELS(I)%EMITTED_MACRO_COUNT_WINDOW = 0_8
         SECONDARY_EMISSION_MODELS(I)%EMITTED_WEIGHT_WINDOW = 0.d0
         SECONDARY_EMISSION_MODELS(I)%OUT_OF_RANGE_COUNT = 0_8
      END DO
      SURFACE_DIAGNOSTIC_WINDOW_STEPS = 0

      DEALLOCATE(LOCAL_DATA, TOTAL_DATA)
   END SUBROUTINE SURFACE_EMISSION_SAVE
   ...
   SUBROUTINE BOUNDARY_SAVE
      ...
      CALL SAVE_EXTRACTION_DIAGNOSTICS
   ...
   SUBROUTINE SAVE_EXTRACTION_DIAGNOSTICS

      IMPLICIT NONE

      INTEGER :: NVALUES, I, J, K, IOS
      REAL(KIND=8) :: WINDOW_SECONDS, CURRENT_A, CURRENT_DENSITY
      CHARACTER(LEN=512) :: FILE_NAME
      CHARACTER(LEN=12) :: DIRECTION_NAME

      IF (EXTRACTION_GROUP <= 0 .OR. .NOT. ALLOCATED(EXTRACTION_COUNT)) RETURN

      NVALUES = 2*PARTICLE_SOURCE_TAG_COUNT*N_SPECIES
      IF (PROC_ID == 0) THEN
         CALL MPI_REDUCE(MPI_IN_PLACE, EXTRACTION_COUNT, NVALUES, MPI_INTEGER8, MPI_SUM, 0, MPI_COMM_WORLD, IOS)
         CALL MPI_REDUCE(MPI_IN_PLACE, EXTRACTION_WEIGHT, NVALUES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, IOS)
         CALL MPI_REDUCE(MPI_IN_PLACE, EXTRACTION_CHARGE, NVALUES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, IOS)
      ELSE
         CALL MPI_REDUCE(EXTRACTION_COUNT, EXTRACTION_COUNT, NVALUES, MPI_INTEGER8, MPI_SUM, 0, MPI_COMM_WORLD, IOS)
         CALL MPI_REDUCE(EXTRACTION_WEIGHT, EXTRACTION_WEIGHT, NVALUES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, IOS)
         CALL MPI_REDUCE(EXTRACTION_CHARGE, EXTRACTION_CHARGE, NVALUES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, IOS)
      END IF

      IF (PROC_ID == 0) THEN
         WINDOW_SECONDS = REAL(MAX(0, tID-EXTRACTION_RESET_TIMESTEP), KIND=8)*DT
         WRITE(FILE_NAME,'(A,A,I0,A)') TRIM(ADJUSTL(BOUNDARY_SAVE_PATH)), 'extraction_', tID, '.csv'
         OPEN(54322, FILE=FILE_NAME, STATUS='REPLACE', ACTION='WRITE', IOSTAT=IOS)
         IF (IOS /= 0) THEN
            CALL ERROR_ABORT('Could not open extraction diagnostic output file.')
         ELSE
            WRITE(54322,'(A)') 'direction,source_tag,species,macro_count,macro_weight,charge_C,current_A,current_density_A_m2'
            DO I = 1, 2
               IF (I == 1) THEN
               ...
               END IF
               DO J = 1, PARTICLE_SOURCE_TAG_COUNT
                  DO K = 1, N_SPECIES
                     IF (EXTRACTION_COUNT(I,J,K) == 0_8 .AND. EXTRACTION_WEIGHT(I,J,K) == 0.d0) CYCLE
                     IF (WINDOW_SECONDS > 0.d0) THEN
                        CURRENT_A = EXTRACTION_CHARGE(I,J,K)/WINDOW_SECONDS
                     ELSE
                        CURRENT_A = 0.d0
                     END IF
                     CURRENT_DENSITY = CURRENT_A/EXTRACTION_AREA
                     WRITE(54322,'(A,",",A,",",A,",",I0,",",ES24.16,",",ES24.16,",",ES24.16,",",ES24.16)') &
                        TRIM(DIRECTION_NAME), TRIM(PARTICLE_SOURCE_TAG_NAME(J-1)), TRIM(SPECIES(K)%NAME), &
                        EXTRACTION_COUNT(I,J,K), EXTRACTION_WEIGHT(I,J,K), EXTRACTION_CHARGE(I,J,K), &
                        CURRENT_A, CURRENT_DENSITY
                  END DO
               END DO
            END DO
            CLOSE(54322)
   ...
   END SUBROUTINE SAVE_EXTRACTION_DIAGNOSTICS
   ...
   SUBROUTINE INIT_BOUNDARY_POSTPROCESS
      ...
      IF (EXTRACTION_GROUP > 0) THEN
         ALLOCATE(EXTRACTION_COUNT(2, PARTICLE_SOURCE_TAG_COUNT, N_SPECIES))
         ALLOCATE(EXTRACTION_WEIGHT(2, PARTICLE_SOURCE_TAG_COUNT, N_SPECIES))
         ALLOCATE(EXTRACTION_CHARGE(2, PARTICLE_SOURCE_TAG_COUNT, N_SPECIES))
         EXTRACTION_COUNT = 0_8
         EXTRACTION_WEIGHT = 0.d0
         EXTRACTION_CHARGE = 0.d0
         EXTRACTION_RESET_TIMESTEP = tID
      END IF
      ...
      IF (ALLOCATED(EXTRACTION_COUNT)) THEN
         EXTRACTION_COUNT = 0_8
         EXTRACTION_WEIGHT = 0.d0
         EXTRACTION_CHARGE = 0.d0
         EXTRACTION_RESET_TIMESTEP = tID
   ...
   SUBROUTINE TALLY_CCP_1D_TIMESTEP

      IMPLICIT NONE

      IF (.NOT. CCP1D_DIAGNOSTICS) RETURN
      IF (DIMS .NE. 1) RETURN

      CCP_1D_DIAG_FIELD_POWER_SPECIES_SUM(1:N_SPECIES) = &
      CCP_1D_DIAG_FIELD_POWER_SPECIES_SUM(1:N_SPECIES) + FIELD_POWER_SPECIES(1:N_SPECIES)
      CCP_1D_DIAG_SAMPLE_COUNT = CCP_1D_DIAG_SAMPLE_COUNT + 1

   END SUBROUTINE TALLY_CCP_1D_TIMESTEP


   SUBROUTINE TALLY_CCP_1D_BOUNDARY(PART, BOUND_ID)
      ...
      INTEGER :: JS
      REAL(KIND=8) :: AREA, CFNUM, SPWT, MASS, V2

      IF (.NOT. CCP1D_DIAGNOSTICS) RETURN
      IF (DIMS .NE. 1) RETURN
      IF (BOUND_ID .LT. 1 .OR. BOUND_ID .GT. 2) RETURN
      IF (BOOL_PERIODIC(BOUND_ID)) RETURN

      JS = PART%S_ID
      IF (JS .LT. 1 .OR. JS .GT. 100) RETURN

      AREA = (YMAX-YMIN)*(ZMAX-ZMIN)
      IF (AREA .LE. 0.d0 .OR. DT .LE. 0.d0) RETURN

      IF (BOOL_RADIAL_WEIGHTING) THEN
         CFNUM = CELL_FNUM(PART%IC)
      ELSE
         CFNUM = FNUM
      END IF
      ...
      SPWT = SPECIES(JS)%SPWT
      MASS = SPECIES(JS)%MOLECULAR_MASS
      V2 = PART%VX**2 + PART%VY**2 + PART%VZ**2

      CCP_1D_DIAG_BOUND_NFLUX_IN_SUM(JS,BOUND_ID) = &
      CCP_1D_DIAG_BOUND_NFLUX_IN_SUM(JS,BOUND_ID) + CFNUM*SPWT/AREA/DT
      CCP_1D_DIAG_BOUND_EFLUX_IN_SUM(JS,BOUND_ID) = &
      CCP_1D_DIAG_BOUND_EFLUX_IN_SUM(JS,BOUND_ID) + 0.5d0*CFNUM*SPWT*MASS*V2/AREA/DT

   END SUBROUTINE TALLY_CCP_1D_BOUNDARY


   SUBROUTINE RESET_CCP_1D_DIAGNOSTICS

      IMPLICIT NONE

      CCP_1D_DIAG_FIELD_POWER_SPECIES_SUM = 0.d0
      CCP_1D_DIAG_BOUND_NFLUX_IN_SUM = 0.d0
      CCP_1D_DIAG_BOUND_EFLUX_IN_SUM = 0.d0
      CCP_1D_DIAG_SAMPLE_COUNT = 0

   END SUBROUTINE RESET_CCP_1D_DIAGNOSTICS


   SUBROUTINE SAVE_CCP_1D_DIAGNOSTICS

      IMPLICIT NONE

      INTEGER :: JS
      REAL(KIND=8), DIMENSION(100) :: DIAG_FIELD_POWER_SPECIES_TOT
      REAL(KIND=8), DIMENSION(100,2) :: DIAG_BOUND_NFLUX_IN_TOT
      REAL(KIND=8), DIMENSION(100,2) :: DIAG_BOUND_EFLUX_IN_TOT
      REAL(KIND=8) :: CURRENT_TIME, ELECTRODE_AREA, SAMPLE_COUNT
      REAL(KIND=8) :: ELECTRON_FIELD_POWER, ION_FIELD_POWER
      REAL(KIND=8) :: ION_CURRENT_LEFT, ION_CURRENT_RIGHT
      REAL(KIND=8) :: ION_POWER_LEFT, ION_POWER_RIGHT
      REAL(KIND=8) :: CHARGE_ABS
      ...
      DIAG_FIELD_POWER_SPECIES_TOT = 0.d0
      DIAG_BOUND_NFLUX_IN_TOT = 0.d0
      DIAG_BOUND_EFLUX_IN_TOT = 0.d0

      CALL MPI_REDUCE(CCP_1D_DIAG_FIELD_POWER_SPECIES_SUM, DIAG_FIELD_POWER_SPECIES_TOT, &
      100, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
      CALL MPI_REDUCE(CCP_1D_DIAG_BOUND_NFLUX_IN_SUM, DIAG_BOUND_NFLUX_IN_TOT, &
      200, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
      CALL MPI_REDUCE(CCP_1D_DIAG_BOUND_EFLUX_IN_SUM, DIAG_BOUND_EFLUX_IN_TOT, &
      200, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)

      IF (PROC_ID .EQ. 0) THEN
         CURRENT_TIME = tID*DT
         ELECTRODE_AREA = (YMAX-YMIN)*(ZMAX-ZMIN)
         SAMPLE_COUNT = DBLE(MAX(1, CCP_1D_DIAG_SAMPLE_COUNT))
         ELECTRON_FIELD_POWER = 0.d0
         ION_FIELD_POWER = 0.d0
         ION_CURRENT_LEFT = 0.d0
         ION_CURRENT_RIGHT = 0.d0
         ION_POWER_LEFT = 0.d0
         ION_POWER_RIGHT = 0.d0

         IF (ELECTRODE_AREA .GT. 0.d0) THEN
            DO JS = 1, N_SPECIES
               IF (SPECIES(JS)%CHARGE .LT. -1.d-12) THEN
                  ELECTRON_FIELD_POWER = ELECTRON_FIELD_POWER + &
                  DIAG_FIELD_POWER_SPECIES_TOT(JS)/SAMPLE_COUNT/ELECTRODE_AREA
               ELSE IF (SPECIES(JS)%CHARGE .GT. 1.d-12) THEN
                  CHARGE_ABS = ABS(SPECIES(JS)%CHARGE)
                  ION_FIELD_POWER = ION_FIELD_POWER + &
                  DIAG_FIELD_POWER_SPECIES_TOT(JS)/SAMPLE_COUNT/ELECTRODE_AREA
                  ION_CURRENT_LEFT = ION_CURRENT_LEFT + &
                  QE*CHARGE_ABS*DIAG_BOUND_NFLUX_IN_TOT(JS,1)/SAMPLE_COUNT
                  ION_CURRENT_RIGHT = ION_CURRENT_RIGHT + &
                  QE*CHARGE_ABS*DIAG_BOUND_NFLUX_IN_TOT(JS,2)/SAMPLE_COUNT
                  ION_POWER_LEFT = ION_POWER_LEFT + DIAG_BOUND_EFLUX_IN_TOT(JS,1)/SAMPLE_COUNT
                  ION_POWER_RIGHT = ION_POWER_RIGHT + DIAG_BOUND_EFLUX_IN_TOT(JS,2)/SAMPLE_COUNT
               END IF
            END DO
         END IF

         WRITE(FILE_NAME,'(A, A)') TRIM(ADJUSTL(CHECKS_SAVE_PATH)), 'ccp_1d_diagnostics'
         INQUIRE(FILE=FILE_NAME, EXIST=FILE_EXISTS)
         OPEN(54332, FILE=FILE_NAME, POSITION='append', STATUS='unknown', ACTION='write')
         IF (.NOT. FILE_EXISTS) THEN
            HEADER_STRING = 'time samples electrode_area_m2 electron_field_power_Wm2'
            HEADER_STRING = TRIM(HEADER_STRING) // ' abs_electron_field_power_Wm2 ion_field_power_Wm2'
            HEADER_STRING = TRIM(HEADER_STRING) // ' ion_current_left_Am2 ion_current_right_Am2'
            HEADER_STRING = TRIM(HEADER_STRING) // ' ion_current_electrode_avg_Am2 ion_energy_flux_left_Wm2'
            HEADER_STRING = TRIM(HEADER_STRING) // ' ion_energy_flux_right_Wm2'
            HEADER_STRING = TRIM(HEADER_STRING) // ' ion_energy_flux_electrode_avg_Wm2 ion_energy_flux_sum_Wm2'
            WRITE(54332,*) TRIM(HEADER_STRING)
         END IF
         WRITE(54332,*) CURRENT_TIME, CCP_1D_DIAG_SAMPLE_COUNT, ELECTRODE_AREA, &
         ELECTRON_FIELD_POWER, ABS(ELECTRON_FIELD_POWER), ION_FIELD_POWER, &
         ION_CURRENT_LEFT, ION_CURRENT_RIGHT, 0.5d0*(ION_CURRENT_LEFT+ION_CURRENT_RIGHT), &
         ION_POWER_LEFT, ION_POWER_RIGHT, 0.5d0*(ION_POWER_LEFT+ION_POWER_RIGHT), &
         ION_POWER_LEFT+ION_POWER_RIGHT
         CLOSE(54332)
      END IF

      CALL RESET_CCP_1D_DIAGNOSTICS

   END SUBROUTINE SAVE_CCP_1D_DIAGNOSTICS
   ...
   SUBROUTINE CHECKS
      ...
      IF (PROC_ID .EQ. 0) THEN
         ...
         CALL MPI_REDUCE(MPI_IN_PLACE, FIELD_POWER_SPECIES, N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         ...
         IF (.NOT. FILE_EXISTS) THEN
            ...
            DO JS = 1, N_SPECIES
               HEADER_STRING = TRIM(HEADER_STRING) // ' fp_' // TRIM(SPECIES(JS)%NAME)
            END DO
            HEADER_STRING = TRIM(HEADER_STRING) // ' coilcurrent'
            DO JS = 1, N_REACTIONS
         ...
         FIELD_POWER_TOT, FIELD_POWER_SPECIES(1:N_SPECIES), COIL_CURRENT, TOT_REACT_COUNTS !TOT_FE, TOT_EE
         CLOSE(54331)
      ...
      ELSE
         ...
         CALL MPI_REDUCE(FIELD_POWER, FIELD_POWER_TOT, 1, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(FIELD_POWER_SPECIES, FIELD_POWER_SPECIES, N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, &
         MPI_COMM_WORLD, ierr)
