...
MODULE postprocess
   ...
   SUBROUTINE GRID_AVG
      ...
      IF (PIC_TYPE .NE. NONE .AND. DIMS == 1) TIMESTEP_E2X = E_FIELD(1,1,:)**2
   ...
   SUBROUTINE GRID_SAVE
      ...
      IF (PROC_ID .EQ. 0) THEN
         ...
         CALL SAVE_TURNER_CASE1_GRID_DIAGNOSTICS
   ...
   SUBROUTINE TALLY_TURNER_CASE1_TIMESTEP

      IMPLICIT NONE

      IF (DIMS .NE. 1) RETURN

      TURNER_CASE1_DIAG_FIELD_POWER_SPECIES_SUM(1:N_SPECIES) = &
      TURNER_CASE1_DIAG_FIELD_POWER_SPECIES_SUM(1:N_SPECIES) + FIELD_POWER_SPECIES(1:N_SPECIES)
      TURNER_CASE1_DIAG_SAMPLE_COUNT = TURNER_CASE1_DIAG_SAMPLE_COUNT + 1

   END SUBROUTINE TALLY_TURNER_CASE1_TIMESTEP


   SUBROUTINE TALLY_TURNER_CASE1_BOUNDARY(PART, BOUND_ID)
      ...
      TURNER_CASE1_DIAG_BOUND_NFLUX_IN_SUM(JS,BOUND_ID) = &
      TURNER_CASE1_DIAG_BOUND_NFLUX_IN_SUM(JS,BOUND_ID) + CFNUM*SPWT/AREA/DT
      TURNER_CASE1_DIAG_BOUND_EFLUX_IN_SUM(JS,BOUND_ID) = &
      TURNER_CASE1_DIAG_BOUND_EFLUX_IN_SUM(JS,BOUND_ID) + 0.5d0*CFNUM*SPWT*MASS*V2/AREA/DT

   END SUBROUTINE TALLY_TURNER_CASE1_BOUNDARY


   SUBROUTINE RESET_TURNER_CASE1_DIAGNOSTICS

      IMPLICIT NONE

      TURNER_CASE1_DIAG_FIELD_POWER_SPECIES_SUM = 0.d0
      TURNER_CASE1_DIAG_BOUND_NFLUX_IN_SUM = 0.d0
      TURNER_CASE1_DIAG_BOUND_EFLUX_IN_SUM = 0.d0
      TURNER_CASE1_DIAG_SAMPLE_COUNT = 0

   END SUBROUTINE RESET_TURNER_CASE1_DIAGNOSTICS


   SUBROUTINE SAVE_TURNER_CASE1_GRID_DIAGNOSTICS

      IMPLICIT NONE

      INTEGER :: JS, JC, FIRST, JC_LEFT, JC_RIGHT, N_CENTER, JPAIR
      REAL(KIND=8) :: CFNUM, SPWT, VOL, DENS, DENS_LEFT, DENS_RIGHT
      REAL(KIND=8) :: CENTER_ELECTRON_DENSITY, CENTER_ION_DENSITY
      REAL(KIND=8) :: CENTER_ELECTRON_TEMP_NUM, CENTER_ELECTRON_KBTE_EV
      REAL(KIND=8) :: CENTER_QN_SIGNED_REL, CENTER_QN_ABS_REL
      REAL(KIND=8) :: ION_DENSITY_MIRROR_MAX_REL, DENOM
      ...
      CENTER_ELECTRON_DENSITY = 0.d0
      CENTER_ION_DENSITY = 0.d0
      CENTER_ELECTRON_TEMP_NUM = 0.d0

      DO JC = JC_LEFT, JC_RIGHT
         DO JS = 1, N_SPECIES
            FIRST = 1 + (JS-1)*NCELLS
            SPWT = SPECIES(JS)%SPWT
            ...
            IF (VOL .GT. 0.d0) THEN
               DENS = CFNUM*SPWT*AVG_NP(FIRST+JC-1)/VOL/DBLE(N_CENTER)
               IF (SPECIES(JS)%CHARGE .LT. -1.d-12) THEN
                  CENTER_ELECTRON_DENSITY = CENTER_ELECTRON_DENSITY + DENS
                  CENTER_ELECTRON_TEMP_NUM = CENTER_ELECTRON_TEMP_NUM + &
                  DENS*AVG_TTR(FIRST+JC-1)
               ELSE IF (SPECIES(JS)%CHARGE .GT. 1.d-12) THEN
                  CENTER_ION_DENSITY = CENTER_ION_DENSITY + DENS
               END IF
            END IF
         END DO
      END DO

      CENTER_ELECTRON_KBTE_EV = 0.d0
      IF (CENTER_ELECTRON_DENSITY .GT. 0.d0) THEN
         CENTER_ELECTRON_KBTE_EV = KB*CENTER_ELECTRON_TEMP_NUM/CENTER_ELECTRON_DENSITY/QE
      END IF

      DENOM = MAX(MAX(CENTER_ELECTRON_DENSITY, CENTER_ION_DENSITY), 1.d-300)
      CENTER_QN_SIGNED_REL = (CENTER_ION_DENSITY-CENTER_ELECTRON_DENSITY)/DENOM
      CENTER_QN_ABS_REL = ABS(CENTER_QN_SIGNED_REL)

      ION_DENSITY_MIRROR_MAX_REL = 0.d0
      DO JC = 1, NCELLS/2
         JPAIR = NCELLS + 1 - JC
         DENS_LEFT = 0.d0
         DENS_RIGHT = 0.d0
         ...
         DO JS = 1, N_SPECIES
            IF (SPECIES(JS)%CHARGE .GT. 1.d-12) THEN
               FIRST = 1 + (JS-1)*NCELLS
               SPWT = SPECIES(JS)%SPWT
               ...
               ELSE
                  CFNUM = FNUM
               END IF
               IF (VOL .GT. 0.d0) DENS_LEFT = DENS_LEFT + CFNUM*SPWT*AVG_NP(FIRST+JC-1)/VOL

               IF (GRID_TYPE == RECTILINEAR_UNIFORM) THEN
                  VOL = CELL_VOL
               ELSE IF (GRID_TYPE == RECTILINEAR_NONUNIFORM) THEN
                  VOL = CELL_VOLUMES(JPAIR)
               ELSE IF (GRID_TYPE == UNSTRUCTURED) THEN
                  VOL = U1D_GRID%CELL_VOLUMES(JPAIR)
               ELSE
                  VOL = CELL_VOL
               END IF
               IF (BOOL_RADIAL_WEIGHTING) THEN
                  CFNUM = CELL_FNUM(JPAIR)
               ELSE
                  CFNUM = FNUM
               END IF
               IF (VOL .GT. 0.d0) DENS_RIGHT = DENS_RIGHT + CFNUM*SPWT*AVG_NP(FIRST+JPAIR-1)/VOL
            END IF
         END DO

         DENOM = MAX(MAX(DENS_LEFT, DENS_RIGHT), 1.d-300)
         ION_DENSITY_MIRROR_MAX_REL = MAX(ION_DENSITY_MIRROR_MAX_REL, &
         ABS(DENS_LEFT-DENS_RIGHT)/DENOM)
      END DO

      WRITE(FILE_NAME,'(A, A)') TRIM(ADJUSTL(FLOWFIELD_SAVE_PATH)), 'turner_case1_grid_diagnostics'
      INQUIRE(FILE=FILE_NAME, EXIST=FILE_EXISTS)
      OPEN(54333, FILE=FILE_NAME, POSITION='append', STATUS='unknown', ACTION='write')
      IF (.NOT. FILE_EXISTS) THEN
         HEADER_STRING = 'time center_electron_density_m3 center_ion_density_m3'
         HEADER_STRING = TRIM(HEADER_STRING) // ' center_electron_kbte_eV center_quasineutral_signed_rel'
         HEADER_STRING = TRIM(HEADER_STRING) // ' center_quasineutral_abs_rel ion_density_mirror_max_rel'
         WRITE(54333,*) TRIM(HEADER_STRING)
      END IF
      WRITE(54333,*) tID*DT, CENTER_ELECTRON_DENSITY, CENTER_ION_DENSITY, &
      CENTER_ELECTRON_KBTE_EV, CENTER_QN_SIGNED_REL, CENTER_QN_ABS_REL, ION_DENSITY_MIRROR_MAX_REL
      CLOSE(54333)

   END SUBROUTINE SAVE_TURNER_CASE1_GRID_DIAGNOSTICS
   ...
   SUBROUTINE CHECKS
      ...
      REAL(KIND=8)                       :: ELECTRODE_AREA, SAMPLE_COUNT
      ...
      CALL MPI_REDUCE(TURNER_CASE1_DIAG_FIELD_POWER_SPECIES_SUM, DIAG_FIELD_POWER_SPECIES_TOT, &
      100, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
      CALL MPI_REDUCE(TURNER_CASE1_DIAG_BOUND_NFLUX_IN_SUM, DIAG_BOUND_NFLUX_IN_TOT, &
      200, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
      CALL MPI_REDUCE(TURNER_CASE1_DIAG_BOUND_EFLUX_IN_SUM, DIAG_BOUND_EFLUX_IN_TOT, &
      200, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
      ...
      IF (PROC_ID .EQ. 0) THEN
         ...
         IF (DIMS .EQ. 1) THEN
            ...
            SAMPLE_COUNT = DBLE(MAX(1, TURNER_CASE1_DIAG_SAMPLE_COUNT))
            ...
            WRITE(file_name,'(A, A)') TRIM(ADJUSTL(CHECKS_SAVE_PATH)), 'turner_case1_diagnostics'
            INQUIRE(FILE=file_name, EXIST=FILE_EXISTS)
            ...
            END IF
            WRITE(54332,*) CURRENT_TIME, TURNER_CASE1_DIAG_SAMPLE_COUNT, ELECTRODE_AREA, &
      ...
      CALL RESET_TURNER_CASE1_DIAGNOSTICS
